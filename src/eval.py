...
def main(cfg):
    ...
    if cfg.experiment.export:
        ...
        with torch.no_grad():
            for i, batch in enumerate(data_module.val_dataloader()):
                ...
                torch.onnx.export(
                    model_module,                     
                    (batch,batch),               
                    "/media/ava/DATA3/DATA/raj/GKT/segmentation/artifacts/gkt.onnx",              
                    verbose=True,
                    export_params=True,         
                    opset_version=16,
                )
            ...
            model = onnx.load("/media/ava/DATA3/DATA/raj/GKT/segmentation/artifacts/gkt.onnx")
            onnx.checker.check_model(model)
            model_simplified, check = simplify(model, check_n=3)
            assert check, 'assert check failed'
            onnx.save(model_simplified, '/media/ava/DATA3/DATA/raj/GKT/segmentation/artifacts/simplified_gkt.onnx')
            print("Checked",check)
            print("Export Successfully...")
            exit()
    elif cfg.experiment.ort:
        with torch.no_grad():
            import onnxruntime as ort
            from tqdm import tqdm
            print("\nOnnxRuntime Inference:\n")
            sess=ort.InferenceSession("/media/ava/DATA3/DATA/raj/GKT/segmentation/artifacts/simplified_gkt.onnx",providers=['CUDAExecutionProvider'])
        ...
        with torch.no_grad():
            ...
            from tqdm import tqdm
            for i, data in enumerate(tqdm(data_module.val_dataloader(), desc="Validation Iterations")):
                data['cam_idx']=data['cam_idx'].cuda()
                data['image']=data['image'].cuda()
                data['intrinsics']=data['intrinsics'].cuda()
                data['extrinsics']=data['extrinsics'].cuda()
                data['bev']=data['bev'].cuda()
                data['view']=data['view'].cuda()
                data['visibility']=data['visibility'].cuda()
                data['center']=data['center'].cuda()
